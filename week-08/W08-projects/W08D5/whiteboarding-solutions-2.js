...
const pathSum = (root, sum, curSum = 0) => {
  if (!root) return false;
  curSum += root.val;
  if (sum === curSum && !root.left && !root.right) return true;
  const leftRecur = pathSum(root.left, sum, curSum);
  const rightRecur = pathSum(root.right, sum, curSum);
