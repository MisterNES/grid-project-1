...
function reverseList(list) {
	// A list with only zero or one node is the same reversed
...
function findNum2(arr) {
	...
	// targetSum will keep track of the sum from 0 to arr.length inclusive
	// actualSum will be the sum of the numbers in arr
	for (let i = 0; i < arr.length; i++) {
		targetSum += i;
		actualSum += arr[i];
	}

	// since our loop can't go upto arr.length because we can't index into an
	// arr at arr.length
	targetSum += arr.length;
	
	// the difference between the targetSum and the actualSum will be the missing
	// number
	let missingNum = targetSum - actualSum;
	return missingNum;
}
